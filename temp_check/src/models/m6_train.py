...
def build_session_window_index(subjects: List[str],
    ...
    if missing_sessions:
        ...
        print(f"⚠️  Warning: Missing embeddings for {missing_names}")
        print(f"   Available: {available_names}")
        print(f"   Skipping subjects without embeddings (proceeding with {len(valid_sessions)} sessions)")
    ...
    if target_size is not None:
        if total == 0 and target_size > 0:
            print(f"⚠️  Warning: Unable to build index - no valid window counts. Using random embeddings.")
            # Return empty index; M6Dataset will use random embeddings as fallback
            return []
        if total != target_size:
            tolerance = max(1, int(0.10 * target_size))  # 10% tolerance for missing embeddings
            if abs(total - target_size) > tolerance:
                print(f"⚠️  Warning: Index size mismatch: {total} vs fold {target_size}")
                print(f"   This can happen with missing embeddings. Proceeding with {total} samples.")
            else:
...
class M6Dataset(Dataset):
    ...
    def __init__(self,
        ...
        if missing_sessions:
            ...
            print(f"⚠️  Warning: Missing embeddings for {missing_sessions}")
            print(f"   Available: {available}")
            print(f"   Will use random embeddings as fallback for missing sessions")

        if self.index is None:
            print(f"⚠️  Warning: No index provided - will use random embeddings for all samples")
        elif len(self.index) != len(self.mm_y):
            print(f"⚠️  Warning: Index length {len(self.index)} != labels length {len(self.mm_y)}")
            print(f"   Will use random embeddings for samples beyond index")

        n_samples = len(self.mm_y)
        n_emb = len(self.embeddings_map)
        print(f"  [info] M6Dataset: {n_samples} samples, {n_emb} session embeddings available")
